...
static char	*get_next_word(const char *str, char c, int *pos)
{
	int		len;
	char	*word;
	int		i;
...
static char	*get_rest_of_string(const char *str, int pos)
{
	int		len;
	char	*word;
	int		i;
...
char	**gc_split_first_word(const char *str, char c)
{
	...
	if (!result)
		return (NULL);
	pos = 0;
	result[0] = get_next_word(str, c, &pos);
	if (str[pos] == c)
	{
		pos++;
		result[1] = get_rest_of_string(str, pos);
	}
	else
	{
		result[1] = gc_malloc(sizeof(char));
		if (result[1])
			result[1][0] = '\0';
	}
	result[2] = NULL;
