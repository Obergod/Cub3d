...
/*   Updated: 2025/11/15 18:35:53 by ufalzone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_map(t_game *game, t_point origin)
{
	...
	while (game->map[y])
	{
		...
		while (game->map[y][x])
		{
			if (game->map[y][x] == '1')
			{
				cell.x = origin.x + x * BLOCKSIZE / 4;
				cell.y = origin.y + y * BLOCKSIZE / 4;
				draw_square(game, cell, BLOCKSIZE / 4, 0xFFFFFF);
			}
			x++;
		}
		y++;
...
void	draw_minimap(t_game *game, t_player *player)
{
	...
	margin = 25;
	origin.x = WIDHT - game->nb_column * (BLOCKSIZE / 4) - margin;
	origin.y = margin;
	area.x = origin.x;
	area.y = origin.y;
	area.w = game->nb_column * BLOCKSIZE / 4;
	area.h = game->nb_lines * BLOCKSIZE / 4;
	fill_rect(game, area);
	draw_map(game, origin);
	marker.x = origin.x + (int)((player->x / BLOCKSIZE) * (BLOCKSIZE / 4));
	marker.y = origin.y + (int)((player->y / BLOCKSIZE) * (BLOCKSIZE / 4));
	marker.x -= 2;
	marker.y -= 2;
